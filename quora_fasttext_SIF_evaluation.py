...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 16 14:36:01 2019
...
checkpoint = torch.load('/afs/inf.ed.ac.uk/user/s18/s1834310/Documents/Dissertation/Models/Quora/FasttextSIF/quora_fastext_SIF14.pth',map_location='cpu')
...
val_set = architectures.SIFDataset('Data/QuoraDuplicateQuestions/quora_val_fasttext_SIF.csv', 300)
...
plt.title('Quora Fasttext Simple Model ROC')
...
len(oov_words)/len(df_vocab)#27.75%
#n oov_words: 29285
#n df_vocab: 105546
...
len(text_complete_oov)/len(text_complete)#1.20%
...
df_vocab = list(set([item for sublist in column1_list for item in sublist] + \
                        [item for sublist in column2_list for item in sublist]))
oov_words = [word for word in df_vocab if word not in fasttext.vocab]
len(oov_words)/len(df_vocab)#19.72
...
text_complete = text_complete1 + text_complete2
text_complete_oov = [word for word in text_complete if word not in fasttext.vocab]
len(text_complete_oov)/len(text_complete)#1.23
...
len(expanded_oov_words)/len(df_vocab)#12.92%
...
len(expanded_text_complete_oov)/len(text_complete)#0.41%
