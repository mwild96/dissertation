...
#TRAIN
#quora_train_bert_SIF = embedder.weighted_average_embedding_array('bert', quora_train.iloc[194088:,:], 'question1', 'question2', question1_weight_dict, question2_weight_dict, tokenizer = bert_tokenizer, vocabulary = None, model = bert_model)

#add labels
#quora_train_bert_SIF = np.concatenate((quora_train_bert_SIF, quora_labels_train.iloc[194088:].values.reshape(-1,1)), axis=1)

#export
#pd.DataFrame(quora_train_bert_SIF).to_csv('Data/QuoraDuplicateQuestions/quora_train_bert_SIF5.csv', header = False, index = False)
