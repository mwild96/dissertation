...
train = pd.read_csv('Data/QuoraDuplicateQuestions/quora_train.csv')
...
#What this answers: do the contextualized word embeddings tend to have a higher oov on their correctly predicted pairs?
...
sns.kdeplot(val_raw.loc[~val_raw.index.isin(word2vec_fn_indices.tolist() + word2vec_fp_indices.tolist()),'word2vec_perct_oov_complete'], color = 'b', label = 'correct predictions word2vec')
sns.kdeplot(val_raw.loc[~val_raw.index.isin(context_only_wrong_indices),'word2vec_perct_oov_complete'], color = 'r', label = 'correct predictions context only')
plt.xlabel('Text Out-of-Vocabulary Percentage (Both Questions)')
plt.ylabel('Density')
plt.show()
...
embedder.expand_vocabulary(fasttext, train, 'question1', 'question2', 300)

val_raw['fasttext_perct_oov_complete'] = val_raw['complete_text'].progress_apply(lambda x: percentage_oov(x, fasttext))
...
sns.kdeplot(val_raw.loc[~val_raw.index.isin(fasttext_fn_indices.tolist() + fasttext_fp_indices.tolist()),'fasttext_perct_oov_complete'], color = 'b', label = 'correct predictions fasttext')
sns.kdeplot(val_raw.loc[~val_raw.index.isin(context_only_wrong_indices),'fasttext_perct_oov_complete'], color = 'r', label = 'correct predictions context only')
plt.xlabel('Text Out-of-Vocabulary Percentage (Both Questions)')
plt.ylabel('Density')
plt.show()
...
#is there any difference between the the way contextualized and non-contextualized perform 
#in the face of polysemous words?
np.median(val_raw.loc[~val_raw.index.isin(non_context_only_wrong_indices)]['polysemous_total'])
np.median(val_raw.loc[~val_raw.index.isin(context_only_wrong_indices)]['polysemous_total'])
#^^no difference...

np.min(val_raw['polysemous_total'])
np.max(val_raw['polysemous_total'])
np.quantile(val_raw['polysemous_total'], 0.99)
sns.kdeplot(val_raw['polysemous_total'])
val_raw['polysemous_total_bins'] = pd.cut(val_raw.loc[val_raw['polysemous_total']<=32,'polysemous_total'], 5,  labels = ['(0,6.4]', '(6.4,12.8]', '(12.8,19.2]', '(19.2,25.6]', '(25.6,126]'])

#assign all outliers to the last bin
val_raw.loc[val_raw['polysemous_total']>32, 'polysemous_total_bins'] = val_raw['polysemous_total_bins'].iloc[5]



ax1 = sns.countplot(x = val_raw.loc[~val_raw.index.isin(non_context_wrong_indices)]['polysemous_total_bins'])
for p, label in zip(ax1.patches, val_raw.loc[~val_raw.index.isin(non_context_wrong_indices)]['polysemous_total_bins'].value_counts()/val_raw['polysemous_total_bins'].value_counts()):
    ax1.annotate(str(np.round(label*100, 1)) + '%', (p.get_x()+0.20, p.get_height()+250))
plt.xlabel('Number of Polysemous Words (Both Questions)')
plt.ylabel('Number of Correctly Classified Pairs')

ax2 = sns.countplot(x = val_raw.loc[~val_raw.index.isin(context_wrong_indices)]['polysemous_total_bins'])
for p, label in zip(ax2.patches, val_raw.loc[~val_raw.index.isin(context_wrong_indices)]['polysemous_total_bins'].value_counts()/val_raw['polysemous_total_bins'].value_counts()):
    ax2.annotate(str(np.round(label*100, 1)) + '%', (p.get_x()+0.20, p.get_height()+250))
plt.xlabel('Number of Polysemous Words (Both Questions)')
plt.ylabel('Number of Correctly Classified Pairs')
